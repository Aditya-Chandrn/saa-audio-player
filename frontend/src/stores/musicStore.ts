...
// Define the type for a song (audio track)
interface Song {
  id: number;
  title: string;
  artist: string;
...
export const musicStore = (() => {
  ...
  return {
    ...
    playPrevious: () => update(state => {
      const prevIndex =
        state.currentIndex === 0
          ? state.playlist.length - 1
          : state.currentIndex - 1; // Loop to end if at the start
      return {
        ...state,
    ...
    setCurrentSong: (index: number) => update(state => {
